...
private[debug] object DebugProtocol {
  ...
  object BreakpointsNotification {
    def unapply(
        notification: NotificationMessage
    ): Option[BreakpointEventArguments] = {
      if (notification.getMethod != "breakpoint") None
      else {
        notification.getParams match {
          case json: JsonElement => json.as[BreakpointEventArguments].toOption
          case _ => None
        }
      }
    }
