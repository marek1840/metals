...
object Debuggee {
  ...
  def factory(
    ...
    sessionParams.dataKind match {
      case RunParamsDataKind.SCALA_MAIN_CLASS =>
        sessionParams.data.as[ScalaMainClass].map {
          mainClass =>
            { id: String =>
      ...
      case TestParamsDataKind.SCALA_TEST =>
        sessionParams.data.as[ScalaTestParams].map {
          testParams =>
            { id: String =>
